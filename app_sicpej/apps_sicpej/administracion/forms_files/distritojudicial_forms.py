...
from ..models import DistritoJudicial, Materia, Municipio

class DistritoJudicialForm(forms.ModelForm):
    estatus = forms.ChoiceField(
        choices=[('', '---------')] + list(DistritoJudicial.ESTATUS),
        label='Estatus',
        required=True,
    ...
    class Meta:
        ...
        fields = ['nombre', 'descripcion','locacion','municipio','materia','estatus']
        widgets = {
    ...
    def clean_nombre(self):
        ...
        if DistritoJudicial.objects.filter(nombre__iexact=nombre).exclude(pk=self.instance.pk).exists():
